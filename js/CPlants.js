...
oLawnCleaner = InheritO(CPlants, {
    ...
    Tooltip: "最普通的草地剪草机",
    NormalAttack: function(a) { (function(j, c, b, d, h, e) {
...
oBrains = InheritO(CPlants, {
    ...
    PrivateBirth: function(a) {
        ...
        function(d) {
            var c, b; (b = --oS.BrainsNum) ? (c = b * d.DieStep, $("imgFlagHead").style.left = (c - 11) + "px", $("imgFlagMeterFull").style.clip = "rect(0,157px,21px," + c + "px)") : ($("imgFlagHead").style.left = "-1px", $("imgFlagMeterFull").style.clip = "rect(0,157px,21px,0)", oP.FlagToEnd())
        }) : function(b) {
            GameOver()
        }
    },
...
oSunFlower = InheritO(CPlants, {
    ...
    PrivateBirth: function(a) {
        oS.ProduceSun ? oSym.addTask(600,
        function(d, c, b) {
        ...
        [a.id, GetX(a.C) - 40, GetY(a.R)]) : a.getHurt = function(e, c, b) {
            ...
            case 0:
                ...
                [d.C, d.R]); (d.HP -= b) < 1 ? d.Die() : oSym.addTask(50,
                function(g, f) {
                    AppearSun(Math.floor(GetX(g) - 40 + Math.random() * 41), GetY(f), 25, 0)
                },
                [d.C, d.R]);
...
oSquash = InheritO(CPlants, {
    ...
    TriggerCheck: function(h, g, e) {
        ...
        a = $(b).childNodes[1],
        f = h.isAttacking;
        h.beAttacked && h.Altitude > -1 && h.Altitude < 2 && (f || !f && c - this.AttackedRX < 71) && (oT.$[this.R].splice(e, 1), a.src = c > this.AttackedRX ? "images/Plants/Squash/SquashR.png": "images/Plants/Squash/SquashL.png", oSym.addTask(100,
        function(d, j, i) {
            var k = $P[d];
    ...
    NormalAttack: function(d, c, b) {
        ...
        function(f, l, j) {
            ...
            h = g.length,
            k;
            while (h--) { (k = g[h]).Altitude > -1 && k.Altitude < 3 && k.getHurt(0, 0, 1800, 0, 0, 0, 2)
            }
